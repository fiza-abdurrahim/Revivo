(function($, window) {
    ...
    $.formUtils.addValidator({
        ...
        validatorFunction: function(value, $el, config, language, $form) {
            var password, passwordInputName = $el.valAttr('confirm') || ($el.attr('name') + '_confirmation'), $passwordInput = $form.find('[name="' + passwordInputName + '"]');
            if (!$passwordInput.length) {
                $.formUtils.warn('Password confirmation validator: could not find an input ' + 'with name "' + passwordInputName + '"', true);
                return false;
            }
        ...
        $forms.each(function() {
            ...
            if (config) {
                $('[data-validation~="recaptcha"]', $form).each(function() {
                    ...
                    $input.valAttr('recaptcha-widget-id', widgetId).hide().on('beforeValidation', function(evt) {
